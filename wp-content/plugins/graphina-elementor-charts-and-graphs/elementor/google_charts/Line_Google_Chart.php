<?php
/**
 * Google line chart elementor widget class.
 *
 ...
 *
 * Elementor widget that displays an eye-catching line bar chart.
 */
	...
	protected function render(): void {
		...
		if ( $settings[ 'iq_' . $type . '_chart_data_option' ] === 'manual' ) {
			...
			foreach ( $settings[ 'iq_' . $type . '_category_list' ] as $key => $value ) {
				$line_data[ $key ][] = esc_html( $xaxis_prefix . $value[ 'iq_' . $type . '_chart_category' ] . $xaxis_postfix );
			}
		}
		...
		for ( $j = 0; $j < $settings[ 'iq_' . $type . '_chart_data_series_count' ]; $j++ ) {
			...
			$point_show            = esc_html( $settings[ 'iq_' . $type . '_chart_point_show' . $j ] );
			if ( $point_show ) {
				$point_size  = $settings[ 'iq_' . $type . '_chart_line_point_size' . $j ];
				$point_shape = $settings[ 'iq_' . $type . '_chart_line_point' . $j ];
			} else {
				$point_size  = null;
				$point_shape = null;
			}

			$line_dash = match ( $settings[ 'iq_' . $type . '_chart_element_lineDash' . $j ] ) {
				'style_1' => array( 1, 1 ),
				'style_2' => array( 2, 2 ),
				'style_3' => array( 4, 4 ),
				'style_4' => array( 5, 1, 3 ),
			...
			if ( $settings[ 'iq_' . $type . '_chart_data_option' ] === 'manual' ) {
				if ( ! empty( $value_list ) && count( $value_list ) > 0 ) {
					foreach ( $value_list as $key => $value ) {
						...
						$line_data[ $key ][] = $value_data;
						if ( $settings[ 'iq_' . $type . '_chart_annotation_show' ] === 'yes' ) {
							$line_data[ $key ][] = esc_html( $annotation_prefix . $value_data . $annotation_postfix );
						}
					}
				}
			}
		...
		if ( graphina_restricted_access( $type, $main_id, $settings, false ) === false ) {
			...
			<script type='text/javascript'>
				...
				(function() {
					'use strict';
					function renderGoogleLineChart(){
					...
					function drawChart() {
						...
						data.addRows(<?php echo wp_json_encode( $line_data ); ?>);
						...
						var options = {
							...
							curveType: '<?php echo esc_js( $settings[ 'iq_' . $type . '_chart_line_curve' ] ); ?>', //function & none
							series: <?php echo wp_json_encode( $series_style_array ); ?>,
							isStacked: false,
							annotations: {
							...
							vAxis: {
								...
								logScale:'<?php echo esc_js( $settings[ 'iq_' . $type . '_chart_logscale_show' ] ); ?>',
						...
						if (typeof graphinaGoogleChartInit !== "undefined") {
							graphinaGoogleChartInit(
								document.getElementById('line_google_chart<?php echo esc_js( $main_id ); ?>'),
								{
									ele:document.getElementById('line_google_chart<?php echo esc_js( $main_id ); ?>'),
									options: options,
									series: data,
									animation: true,
