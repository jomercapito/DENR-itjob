<?php
/**
 * Google column chart elementor widget class.
 *
 ...
 *
 * Elementor widget that displays an eye-catching Google column chart.
 */
class Column_Google_Chart extends Widget_Base {
	...
	protected function register_controls(): void {
		...
		graphina_column_chart_google_series_setting( $this, $type, array( 'tooltip', 'color' ) );
	...
	protected function render() {
		...
		if ( $settings[ 'iq_' . $type . '_chart_data_option' ] === 'manual' ) {
			...
			foreach ( $settings[ 'iq_' . $type . '_category_list' ] as $key => $value ) {
				$column_data[ $key ][] = esc_html( $xaxis_prefix . $value[ 'iq_' . $type . '_chart_category' ] . $xaxis_postfix );
			}
		}
			...
			if ( $settings[ 'iq_' . $type . '_chart_data_option' ] === 'manual' ) {
				if ( ! empty( $value_list ) && count( $value_list ) > 0 ) {
					foreach ( $value_list as $key => $value ) {
						...
						$column_data[ $key ][] = $value_data;
						if ( $settings[ 'iq_' . $type . '_chart_annotation_show' ] === 'yes' ) {
							$column_data[ $key ][] = esc_html( $annotation_prefix . $value_data . $annotation_postfix );
						}
					}
				}
			}
		...
		$series_style_array = array_fill( 0, count( $column_data ), array( 'targetAxisIndex' => ( $legend_position === 'left' ) ? 1 : 0 ) );
		...
		<script type="text/javascript">
			(function() {
				'use strict';
				function renderGoogleColumnChart(){
				...
				function drawChart() {
					...
					data.addRows(<?php echo wp_json_encode( $column_data ); ?>);
					...
					if (typeof graphinaGoogleChartInit !== "undefined") {
						graphinaGoogleChartInit(
							document.getElementById('column_google_chart<?php echo esc_js( $main_id ); ?>'),
							{
								ele:document.getElementById('column_google_chart<?php echo esc_js( $main_id ); ?>'),
								options: options,
								series: data,
								animation: true,
